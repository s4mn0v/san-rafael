<template>
  <div class="flex h-screen flex-col md:flex-row bg-[var(--color-custom-500)] dark:bg-[var(--color-custom-50)]">
    <!-- Sidebar -->
    <div
      class="flex h-full flex-col items-center gap-4 mx-4 py-8 md:px-2 text-[var(--color-custom-50)] dark:text-[var(--color-custom-500)]">
      ...
      <div class="flex flex-col items-center gap-2 mb-8">
        ...
        <span class="font-bold tracking-widest uppercase text-2xl">San. Rafael</span>
      </div>
      ...
      <NavButtons icon="i-healthicons-agriculture-worker" text="Proveedores" to="/providers" />
      <NavButtons icon="i-healthicons-syringe" text="Reproducción" to="/reproduction" />
      <NavButtons icon="i-healthicons-biomarker-outline" text="Genealogía" to="/genealogy" />
      ...
      <!-- Logout & Theme -->
      <div class="mt-auto flex flex-row items-center gap-4 justify-between w-full">
        <Logout />
        <Theming />
      </div>
    </div>
    ...
    <!-- Main content -->
    <div
      class="m-4 mb-16 flex-1 overflow-auto rounded-xl p-6 md:m-4 md:pb-4 bg-[var(--color-custom-50)] dark:bg-[var(--color-custom-500)]">
      <slot />
    </div>
  </div>
