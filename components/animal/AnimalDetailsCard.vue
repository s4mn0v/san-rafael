      ...
      <div class="grid md:grid-cols-2 gap-6 print:grid print:grid-cols-2">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <p class="text-lg font-semibold">
              {{ hasSaleRecord ? 'Con información de venta' : 'Sin información de venta disponible' }}
            </p>
          </div>
        </div>
      </div>
          ...
          <UFormField label="En Venta" name="venta">
            <div>
              <template v-if="hasSaleRecord">
                <p class="font-semibold">Este registro ya posee información de venta</p>
              </template>

              <template v-else>
                <SaleModal :animal-id="animal.id_animal" v-slot="{ open }" @created="handleSaleCreated">
                  <UButton @click="open"
